...
import sys
sys.path.insert(0, '/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/venv/lib/python3.9/site-packages')
...
class Twibot20(Dataset):
    def __init__(self,root='/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/datatwk/',device='cpu',process=False,save=False):
        self.root = root
        self.device = device
        self.process=process
        if process:
            print('Loading train.json')
            df_train=pd.read_json('/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/Twi-bot20/train.json')
            print('Loading test.json')
            df_test=pd.read_json('/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/Twi-bot20/test.json')
            #print('Loading support.json')
            #df_support=pd.read_json('./Twibot-20/support.json')
            print('Loading dev.json')
            df_dev=pd.read_json('/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/Twi-bot20/dev.json')
            print('Finished')
    ...
    def load_labels(self):
        ...
        if not os.path.exists(path):
            ...
            if self.save:
                torch.save(labels,'/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/datatwk/label.pt')
        else:
    ...
    def Des_embbeding(self):
        ...
        if not os.path.exists(path):
            ...
            print('Loading BERT')
            feature_extraction = pipeline('feature-extraction', model="distilbert-base-uncased", tokenizer="distilbert-base-uncased",device=0)
            des_vec=[]
            ...
            if self.save:
                torch.save(des_tensor,'/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/datatwk/des_tensor.pt')
        else:
    ...
    def tweets_embedding(self):
        ...
        if not os.path.exists(path):
            tweets=np.load("/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/datatwk/tweets.npy",allow_pickle=True)
            print('Loading BERT')
            feature_extract=pipeline('feature-extraction',model='bert-base-uncased',tokenizer='bert-base-uncased',device=0,padding=True, truncation=True,max_length=500, add_special_tokens = True)
            tweets_list=[]
    ...
    def num_prop_preprocess(self):
        ...
        if not os.path.exists(path0):
            ...
            if self.save:
                torch.save(num_prop,"/work/08641/ishannad/ls6/Virtual_Environment/Proj_GNN/datatwk/num_properties_tensor.pt")
...
dataset=Twibot20(device='cuda' ,process=False,save=False)
